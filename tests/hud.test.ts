...
describe('game hud', () => {
  ...
  it('shows the current gun magazine and a cooldown ring', () => {
    ...
    expect(html).toContain('data-hot-index="0"')
    expect(model.hotbar.some((slot) => slot?.itemId === 'pistol' && slot.equipped)).toBe(true)
    expect(model.hotbar.some((slot) => slot?.itemId === 'meal')).toBe(true)
    expect(model.hotbar.some((slot) => slot?.itemId === 'bandage')).toBe(true)

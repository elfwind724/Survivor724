...
import { nightRepairing } from '@/combat/Night'
...
export function activityCaption(world: WorldState, survivor: SurvivorState): string {
  ...
  if (nightRepairing(world, survivor)) return '抢修围墙'
  ...
  if (isScoopingWater(world, survivor)) return '舀水中'
...
export function activityCooldown(world: WorldState, survivor: SurvivorState): number {
  ...
  if (isScoopingWater(world, survivor)) return remainingScoop(world, survivor)
  if (survivor.workerState === 'Work') {
    ...
    if (jobId(world, survivor) === 'draw') return remainingScoop(world, survivor)

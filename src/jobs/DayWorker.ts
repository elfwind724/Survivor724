...
function startNextAction(world: WorldState, survivor: SurvivorState): void {
  ...
  if (definition.id === 'fish') {
    ...
    const target = hole?.position ?? node?.position
    if (!target) {
      goHome(world, survivor)
      return
    }
    if (distanceXZ(survivor.position, target) <= 1.4) {
  ...
  if (definition.id === 'scavenge') {
    ...
    const crate = box?.position ?? node?.position
    if (!crate) {
      goHome(world, survivor)
      return
    }
    if (distanceXZ(survivor.position, crate) <= 1.4) {
      survivor.workElapsed = 0
      survivor.workerState = 'Work'
      survivor.destination = null
      survivor.path = []
    ...
    if (beginTravel(world, survivor, crate)) survivor.workerState = 'TravelToTarget'
    return
  }
  ...
  if (definition.id === 'gather') {
    ...
    const target = shrub?.position ?? node?.position
    if (!target) {
      goHome(world, survivor)
      return
    }
...
function stepWork(world: WorldState, survivor: SurvivorState, dt: number): void {
  const job = currentJob(world, survivor)
  if (job && jobDefinition(job.definitionId)?.id === 'fish') {
    const result = stepFishing(world, survivor, dt, { autoTravel: true })
    if (result === 'travel') survivor.workerState = 'TravelToTarget'
    else if (result === 'full' || (result === 'caught' && shouldReturn(world, survivor))) beginReturn(world, survivor)
    else if (result === 'none' && !hasRequiredTools(survivor, 'fish')) {
      survivor.blockedReason = 'missing_tool'
      beginReturn(world, survivor)
    }
    return
  }
  if (job && jobDefinition(job.definitionId)?.id === 'scavenge') {
    const result = stepScavenge(world, survivor, dt, { autoTravel: true })
    if (result === 'travel') survivor.workerState = 'TravelToTarget'
    else if (result === 'full' || (result === 'looted' && shouldReturn(world, survivor))) beginReturn(world, survivor)
    else if (result === 'empty') beginReturn(world, survivor)
    else if (result === 'none' && !hasRequiredTools(survivor, 'scavenge')) {
      survivor.blockedReason = 'missing_tool'
      beginReturn(world, survivor)
    }
    return
  }
  if (job && jobDefinition(job.definitionId)?.id === 'gather') {
    const result = stepGather(world, survivor, dt, { autoTravel: true })
    if (result === 'travel') survivor.workerState = 'TravelToTarget'
    else if (result === 'full' || (result === 'picked' && shouldReturn(world, survivor))) beginReturn(world, survivor)
    else if (result === 'empty') beginReturn(world, survivor)
    return
  }
